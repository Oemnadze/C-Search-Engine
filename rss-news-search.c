...
static void BuildIndices(const char *feedsFileName, hashset* stopList, hashset* visitedArticles, hashset* foundWords);
static void ProcessFeed(const char *remoteDocumentName, hashset* stopList, hashset* visitedArticles, hashset* foundWords);
static void PullAllNewsItems(urlconnection *urlconn, hashset* stopList, hashset* visitedArticles, hashset* foundWords);
static bool GetNextItemTag(streamtokenizer *st);
static void ProcessSingleNewsItem(streamtokenizer *st, hashset* stopList, hashset* visitedArticles, hashset* foundWords);
static void ExtractElement(streamtokenizer *st, const char *htmlTag, char dataBuffer[], int bufferLength);
static void ParseArticle(const char *articleTitle, const char *articleDescription, const char *articleURL, hashset* stopList, hashset* foundWords);
static void ScanArticle(streamtokenizer *st, const char *articleTitle, const char *unused, const char *articleURL, hashset* stopList, hashset* foundWords);
static void QueryIndices(hashset* stopList, hashset* foundWords);
static void ProcessResponse(const char *word, hashset* stopList, hashset* foundWords);
...
static const char *const stopWordsAddress = "data/stop-words.txt";
static const char *const stopWordsDelimiters = "\n\r";
...
static void stringFreeFunc(void* toFree) {
  char* tofree = *(char**)toFree;
  free(tofree);
}

static int stringCompareFunc(const void* a, const void* b) {
  char* A = *(char**)a;
  char* B = *(char**)b;
...
static int stringHashFunc(const void* elem, int numBuckets) {
...
static void articleFreeFunc(void* toFree) {
  article* art = (article*)toFree;
  free(art->articleName);
  free(art->articleUrl);
}

static int articleCompareFunc(const void* a, const void* b) {
  article* A = (article*)a;
  article* B = (article*)b;
  char* name1 = A->articleName;
  char* name2 = B->articleName;
  char* url1 = A->articleUrl;
  char* url2 = B->articleUrl;
  int i = 0;
  int comp1 = strcasecmp(name1, name2);
  int comp2 = 0;
  int comp3 = strcasecmp(url1, url2);
  int length1 = strlen(url1);
  int length2 = strlen(url2);
...
static int articleHashFunc(const void* elem, int numBuckets) {
  article* art = (article*)elem;
  char* url = art->articleUrl;
  int sz = strlen(url);
  int i;
  for (i = 0; i < sz; i++) {
    if (*(url + i) == '/')
  ...
  for (int j = 0; j < strlen(s); j++)  
    hashcode = hashcode * kHashMultiplier + tolower(s[j]);  
  free(s);
  return hashcode % numBuckets;
}

static void vectorAndWordFreeFunc(void* toFree) {
  vectorAndWord* vcwrd = (vectorAndWord*)toFree;
  free(vcwrd->word);
  VectorDispose(&vcwrd->artCnt);
}

static int vectorAndWordCompareFunc(const void* a, const void* b) {
  vectorAndWord* vcwrd1 = (vectorAndWord*)a;
  vectorAndWord* vcwrd2 = (vectorAndWord*)b;
  char* str1 = vcwrd1->word;
  char* str2 = vcwrd2->word;
  return strcasecmp(str1, str2);
}

static int vectorAndWordHashFunc(const void* elem, int numBuckets) {
  vectorAndWord* vcwrd = (vectorAndWord*)elem;
  char* s = vcwrd->word;
  unsigned long hashcode = 0;
  for (int i = 0; i < strlen(s); i++)  
    hashcode = hashcode * kHashMultiplier + tolower(s[i]);  
...
void fillStopList(hashset* stopList) {
  FILE* input;
  input = fopen(stopWordsAddress, "r");
  streamtokenizer tokens;
  STNew(&tokens, input, stopWordsDelimiters, true);
  char buffer[1024];
  while (true) {
    bool b = STNextToken(&tokens, buffer, 1024); 
    if (!b) {
      break;
    }
    char* stopWord = strdup(buffer);
    HashSetEnter(stopList, &stopWord);
  }
  STDispose(&tokens);
  fclose(input);
}

static void articleAndCntFreeFunc(void* toFree) {
  articleAndCnt* artcnt = (articleAndCnt*)toFree;
  free(artcnt->art.articleName);
  free(artcnt->art.articleUrl);
}

static int articleAndCntCompareFunc(const void* a, const void* b) {
  articleAndCnt* artCnt1 = (articleAndCnt*)a;
  articleAndCnt* artCnt2 = (articleAndCnt*)b;
  char* str1 = artCnt1->art.articleName;
  char* str2 = artCnt2->art.articleName;
  char* url1 = artCnt1->art.articleUrl;
  char* url2 = artCnt2->art.articleUrl;
  int comp1 = strcasecmp(str1, str2);
  int comp2 = strcasecmp(url1, url2);
  if (comp1 == 0 && comp2 == 0)
    return 0;
  return -1;
}

void foundWordsInsert(hashset* foundWords, char* word1, const char* articleTitle, const char* articleURL) {
  vectorAndWord vcWrd;
  vcWrd.word = strdup(word1);
  vectorAndWord* ptr = HashSetLookup(foundWords, &vcWrd);
  if (ptr == NULL) {
    vectorAndWord vcword;
    vcword.word = strdup(word1);
    VectorNew(&vcword.artCnt, sizeof(articleAndCnt), articleAndCntFreeFunc, 10);
    HashSetEnter(foundWords, &vcword);
    ptr = HashSetLookup(foundWords, &vcword);
  }
  free(vcWrd.word);
  article artc;
  artc.articleName = articleTitle;
  artc.articleUrl = articleURL;
  articleAndCnt artCnt;
  artCnt.art = artc;
  artCnt.cnt = 0;
  int indx = VectorSearch(&ptr->artCnt, &artCnt, articleAndCntCompareFunc, 0, false);
  if (indx != -1) {
    articleAndCnt* ptr1;
    ptr1 = VectorNth(&ptr->artCnt, indx);
    ptr1->cnt++;
  }
  else {
    articleAndCnt ptr1;
    ptr1.cnt = 1;
    ptr1.art.articleName = strdup(articleTitle);
    ptr1.art.articleUrl = strdup(articleURL);
    VectorAppend(&ptr->artCnt, &ptr1);
  }
}

static int vectorCompFunc(const void* a, const void* b) {
  articleAndCnt* acn1 = (articleAndCnt*)a;
  articleAndCnt* acn2 = (articleAndCnt*)b;
  int cnt1 = acn1->cnt;
  int cnt2 = acn2->cnt;
  if (cnt1 > cnt2) 
    return -1;
...
void printArticleCnts(hashset* foundWords, const char* word) {
  vectorAndWord vcWrd;
  vcWrd.word = word;
  vectorAndWord* vcWord = (vectorAndWord*)HashSetLookup(foundWords, &vcWrd);
  int sz = VectorLength(&vcWord->artCnt);
  VectorSort(&vcWord->artCnt, vectorCompFunc); 
  if (sz > 10)
    sz = 10;
  for (int i = 0; i < sz; i++) {
    articleAndCnt* acn = VectorNth(&vcWord->artCnt, i);
    if (acn->cnt != 1)
      printf("%d.) \"%s\" [search term occurs %d times]\n \"%s\"\n", i + 1, acn->art.articleName, acn->cnt, acn->art.articleUrl);
    else
...
int main(int argc, char **argv)
{
  ...
  hashset stopList;
  HashSetNew(&stopList, sizeof(char*), 1009, stringHashFunc, stringCompareFunc, stringFreeFunc);
  fillStopList(&stopList);

  hashset visitedArticles;
  HashSetNew(&visitedArticles, sizeof(article), 10007, articleHashFunc, articleCompareFunc, articleFreeFunc);

  hashset foundWords;
  HashSetNew(&foundWords, sizeof(vectorAndWord), 10007, vectorAndWordHashFunc, vectorAndWordCompareFunc, vectorAndWordFreeFunc);

  setbuf(stdout, NULL);
  Welcome(kWelcomeTextFile);
  BuildIndices((argc == 1) ? kDefaultFeedsFile : argv[1], &stopList, &visitedArticles, &foundWords);

  QueryIndices(&stopList, &foundWords);

  HashSetDispose(&stopList);
  HashSetDispose(&visitedArticles);
  HashSetDispose(&foundWords);
...
static void BuildIndices(const char *feedsFileName, hashset* stopList, hashset* visitedArticles, hashset* foundWords)
{
  ...
  while (STSkipUntil(&st, ":") != EOF) { // ignore everything up to the first selicolon of the line
    ...
    ProcessFeed(remoteFileName, stopList, visitedArticles, foundWords);
  }
  STDispose(&st);
  fclose(infile);
  printf("\n");
...
static void ProcessFeedFromFile(char *fileName, hashset* stopList, hashset* visitedArticles, hashset* foundWords)
{
  FILE *infile;
  streamtokenizer st;
  ...
  article art;
  art.articleName = strdup(fileName);
  art.articleUrl = strdup(fileName);
  article* ptr = HashSetLookup(visitedArticles, &art);
  if (ptr == NULL) {
    ScanArticle(&st, (const char *)fileName, articleDescription, (const char *)fileName, stopList, foundWords); 
    HashSetEnter(visitedArticles, &art);
  }
  else {
    free(art.articleName);
    free(art.articleUrl);
...
static void ProcessFeed(const char *remoteDocumentName, hashset* stopList, hashset* visitedArticles, hashset* foundWords)
{
  ...
  if(!strncmp(filePrefix, remoteDocumentName, strlen(filePrefix))){
    ProcessFeedFromFile((char *)remoteDocumentName + strlen(filePrefix), stopList, visitedArticles, foundWords);
    return;
  }
  url u;
  ...
  switch (urlconn.responseCode) {
      case 0: printf("Unable to connect to \"%s\".  Ignoring...", u.serverName);
              break;
      case 200: PullAllNewsItems(&urlconn, stopList, visitedArticles, foundWords);
                break;
      case 301: 
      case 302: ProcessFeed(urlconn.newUrl, stopList, visitedArticles, foundWords);
                break;
...
static void PullAllNewsItems(urlconnection *urlconn, hashset* stopList, hashset* visitedArticles, hashset* foundWords)
{
  ...
  while (GetNextItemTag(&st)) { // if true is returned, then assume that <item ...> has just been read and pulled from the data stream
    ProcessSingleNewsItem(&st, stopList, visitedArticles, foundWords);
  }
...
static void ProcessSingleNewsItem(streamtokenizer *st, hashset* stopList, hashset* visitedArticles, hashset* foundWords)
{
  ...
  article art;
  art.articleName = strdup(articleTitle);
  art.articleUrl = strdup(articleURL);
  if (HashSetLookup(visitedArticles, &art) == NULL) {
    ParseArticle(articleTitle, articleDescription, articleURL, stopList, foundWords);
    HashSetEnter(visitedArticles, &art);
  }
  else {
    free(art.articleName);
...
static void ParseArticle(const char *articleTitle, const char *articleDescription, const char *articleURL, hashset* stopList, hashset* foundWords)
{
  ...
  switch (urlconn.responseCode) {
      ...
      case 200: printf("Scanning \"%s\" from \"http://%s\"\n", articleTitle, u.serverName);
	        STNew(&st, urlconn.dataStream, kTextDelimiters, false);
		ScanArticle(&st, articleTitle, articleDescription, articleURL, stopList, foundWords);
		STDispose(&st);
		break;
      case 301:
      case 302: // just pretend we have the redirected URL all along, though index using the new URL and not the old one...
                ParseArticle(articleTitle, articleDescription, urlconn.newUrl, stopList, foundWords);
		break;
  ...
  while (STNextToken(st, word, sizeof(word))) {
    if (strcasecmp(word, "<") == 0) {
      ...
      if (WordIsWellFormed(word)) {
	      numWords++;
        char* wordPtr = word;
        if (HashSetLookup(stopList, &wordPtr) == NULL) {
          foundWordsInsert(foundWords, word, articleTitle, articleURL);
        }
...
static void QueryIndices(hashset* stopList, hashset* foundWords)
{
  ...
  while (true) {
    ...
    ProcessResponse(response, stopList, foundWords);
  }
}
...
static void ProcessResponse(const char *word, hashset* stopList, hashset* foundWords)
{
  if (WordIsWellFormed(word)) {
    if (HashSetLookup(stopList, &word) != NULL) {
      ...
      if (HashSetLookup(foundWords, &word) == NULL) {
        printf("None of today's news articles contain the word \"%s\".\n", word);
      }
      else {
        printArticleCnts(foundWords, word);
      }
    }
  } else {
